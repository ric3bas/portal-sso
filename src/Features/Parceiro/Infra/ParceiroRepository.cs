...
namespace Portal.Features.Parceiro.Infra {
    public class ParceiroRepository : DapperRepository<ParceiroEntity>, IDapperRepository<ParceiroEntity>, IParceiroRepository {
        ...
        public async Task<IEnumerable<ParceiroResponse>> ObterTodosAsync(string? nome, CancellationToken cancellationToken = default) {
            ...
            var result = await _unitOfWork.Connection.QueryAsync<ParceiroEntity>(sql, param);
        ...
        public async Task<ParceiroResponse?> ObterPorIdAsync(Guid id, CancellationToken cancellationToken = default) {
            ...
            var result = await _unitOfWork.Connection.QuerySingleOrDefaultAsync<ParceiroEntity>(sql, new { Id = id });
        ...
        public async Task<ParceiroResponse?> ObterPorNomeAsync(string nome, CancellationToken cancellationToken) {
            ...
            var result = await _unitOfWork.Connection.QuerySingleOrDefaultAsync<ParceiroEntity>(sql, new { Nome = nome });
            return result?.ToResponse();
        }
        ...
        public async Task<Guid> InserirAsync(ParceiroEntity parceiro, CancellationToken cancellationToken = default) {
            ...
            await _unitOfWork.Connection.ExecuteAsync(sql, parceiro, _unitOfWork.Transaction);
            return parceiro.Id;
        }
