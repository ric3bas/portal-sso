...
public static class PerfilEndpoints
{
    public static void MapPerfilEndpoints(this WebApplication app)
    {
        ...
        group.MapGet("/", async ([FromServices] ObterPerfisComEscopoHandler handler, [AsParameters] ObterPerfisComEscopoRequest request, CancellationToken cancellationToken = default) =>
        {
            var result = await handler.Handle(request, cancellationToken);
            return result.ToHttpResult();
        })
        .RequireScopes("perfil.ler");

        group.MapGet("/combo", async ([FromServices] ObterPerfisParaComboHandler handler, [AsParameters] ObterPerfisParaComboRequest request, CancellationToken cancellationToken = default) =>
        {
            var result = await handler.Handle(request, cancellationToken);
            return result.ToHttpResult();
        })
        ...
        .RequireScopes("perfil.atualizar");
