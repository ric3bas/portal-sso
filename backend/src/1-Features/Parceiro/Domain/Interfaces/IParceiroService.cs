...
namespace Portal.Features.Parceiro.Domain.Interfaces {
    public interface IParceiroService {
        Task<Result<IEnumerable<ParceiroResponse>>> ObterTodosPorFiltroAsync(string? nome, CancellationToken cancellationToken);
        Task<Result<IEnumerable<ParceiroResponse>>> ObterTodosParceirosAsync(CancellationToken cancellationToken);
