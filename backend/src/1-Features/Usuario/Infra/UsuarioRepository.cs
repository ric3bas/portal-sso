...
namespace Portal.Features.Usuario.Infra
{
    ...
    public class UsuarioRepository : DapperRepository, IDapperRepository, IUsuarioRepository
    {
        ...
        public async Task<bool> VerificarUsuarioMasterAsyunc(string usuarioLogin, CancellationToken cancellationToken = default)
        {
            cancellationToken.ThrowIfCancellationRequested();
            const string sql = @"SELECT 1
                                FROM sso.usuario u
                                inner join sso.perfil p
                                on p.id = u.perfil_id
                                where upper(p.nome) = 'MASTER'
                                and u.login = @usuarioLogin";
            var result = await QuerySingleAsync<int>(sql, new { usuarioLogin });
            return result == 1;
        }
        ...
        public async Task<UsuarioQuery?> ObterPorIdAsync(int usuarioId, CancellationToken cancellationToken = default)
        {
            ...
            const string sql = "SELECT id, nome, email, login, senha, parceiro_id, perfil_id, tentativas_login FROM sso.usuario WHERE id = @usuarioId";
