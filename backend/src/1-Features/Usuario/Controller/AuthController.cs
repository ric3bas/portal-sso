...
namespace Portal.Features.Usuario.Controller
{
    ...
    public class AuthController : BaseController
    {
        ...
        [ProducesBadRequestProblem]
        public async Task<IActionResult> EsqueceSenhaAsync([FromBody] RecuperarSenhaRequest request, CancellationToken cancellationToken)
            => HandleResult(await _authService.EsqueceSenhaAsync(request, cancellationToken));


        [SwaggerOperation(Summary = "Solicita token para trocar de senha")]
        [HttpPost("esqueceu-senha-logado")]
        [ProducesResponseType(typeof(string), 200)]
        [ProducesBadRequestProblem]
        public async Task<IActionResult> EsqueceSenhaLogadoAsync([FromBody] RecuperarSenhaRequest request, CancellationToken cancellationToken)
            => HandleResult(await _authService.EsqueceSenhaLogadoAsync(request, cancellationToken));
