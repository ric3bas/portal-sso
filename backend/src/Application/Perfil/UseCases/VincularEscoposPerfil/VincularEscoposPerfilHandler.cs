...
public class VincularEscoposPerfilHandler
{
    ...
    public async Task<Result<VincularEscoposPerfilResponse>> Handle(VincularEscoposPerfilRequest request, CancellationToken cancellationToken)
    {
        if (request.PerfilId <= 0)
            return Result.ValidationResult<VincularEscoposPerfilResponse>("PerfilId inválido");
        ...
        if (!perfilExiste)
            return Result.NotFoundResult<VincularEscoposPerfilResponse>($"Perfil {request.PerfilId} não encontrado");
        ...
        if (idsInvalidos.Count > 0)
            return Result.ValidationResult<VincularEscoposPerfilResponse>($"Os seguintes EscopoIds não existem: {string.Join(", ", idsInvalidos)}");
